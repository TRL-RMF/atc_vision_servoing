...
class AprilTagServo2 {
public:
	AprilTagServo2(ros::NodeHandle& nh_private, std::mutex* mtxPtr);
	virtual ~AprilTagServo2();
	...
	bool isStagingGoalCalculated(){return stagingGoalCalculated;};
	bool isStagingGoalReached(){return stagingGoalReached;};
	bool isDockingGoalReached(){return dockingGoalReached;};
...
private:
	...
	double calculateYawRateCommand(const double& P_yaw, const double& MAX_YAW_RATE_RADSEC, const double& pixelPosRight, bool& hasReached);
	  ...
	  geometry_msgs::PoseStamped marker_pose_map_;
	  geometry_msgs::PoseStamped marker_pose_camera_;
